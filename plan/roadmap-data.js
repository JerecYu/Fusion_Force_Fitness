...
window.FFF_ROADMAP = {
  ...
  meta: {
    updated: '2026-08-17 · GT 上線第二天 · 新增第 44～46 步（逾期提醒、到課共同確認）',
    ...
    /* ══════════ 第五幕 ══════════ */
    {
      ...
      steps: [
        ...
        { n:46, t:'到課共同確認（掃碼簽名）', where:'SQL ＋ 前端', done:true,
          ...
          note:'<b>2026-08-17 完成。</b><br>檔案：<code>db/19-attest.sql</code>（8/16）、<code>db/24-attest-ui.sql</code>、<code>line-prototype/checkin.html</code>、<code>line-prototype/confirm.html</code>、<code>line-prototype/GT-booking.html</code>。<br><br><b>☢️ QR 產生器是自己寫的，內嵌在 <code>checkin.html</code> 裡（約 13 KB）。</b>沒有用 CDN —— 這一頁是教練<b>在場邊、可能訊號很差的時候</b>要開的，多一個外部相依就多一個失敗點，而那個失敗點會剛好在最忙的三十秒發生。<br>只做 byte 模式、糾錯等級 M、版本 1～10。<b>驗證方式</b>：node 產生矩陣 → 畫成圖 → 用 OpenCV 的解碼器掃回來比對，<b>42 個案例（每個版本的容量邊界都測）全過</b>；再用 Playwright 把畫面上真正顯示的那個 QR 截下來解一次，確認掃出來就是正確的 LIFF 網址。<br><br><b>踩到的坑：</b>格式資訊那 15 個位元的落點<b>不是連續的</b>，中間會跳過時序圖案那一格。第一版寫錯 —— 圖看起來完全正常，只是<b>掃不出來</b>，而且沒有任何地方會報錯。',
