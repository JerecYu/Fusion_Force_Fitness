...
window.FFF_ROADMAP = {
  ...
  meta: {
    updated: '2026-08-17 · GT 上線第二天 · 新增第 44～50 步（逾期提醒、到課確認、教練後台、課購、舊表孤兒）',
    ...
    /* ══════════ 第五幕 ══════════ */
    {
      ...
      steps: [
        ...
        { n:48, t:'教練後台：新增客人與綁定診斷', where:'SQL ＋ 前端', done:true,
          ...
          warn:'☢️ <b>這一頁完全不碰堂數。</b>Jerec 2026-08-17 的決定（原話）：<i>「舊系統……不只新增客人還直接可選購買方案，這會有『先上車，但不一定會補票』的問題，所以我選 1 不碰堂數，只建人」</i>。建出來的客人固定 <b>0 堂</b>，堂數走課購流程（C 第二期）。這同時守住第 39 步：<b>動到錢的入口只有一個</b>。<br><br>☢️ <b>「一鍵建客人」故意不做成一鍵。</b>按下去只是把留言簿那筆<b>帶進表單</b>，不直接建 —— 因為姓名那一欄正是最常出問題的地方，要讓教練有機會把「廖庭均 Teresa」改成「廖庭均」再送。<br><br>☢️ <b>登記姓名要短。</b>比對規則是「客人打的要<b>包含</b>登記姓名」：登記「王小明」，他打「王小明 Ming」也會過；登記「王小明 Ming」，他打「王小明」就<b>過不了</b>。8/17 卡住的兩個人都是這個原因，所以這句話直接寫在輸入框底下。<br><br>☢️ <b><code>customers</code> 沒有 INSERT 政策，只能走 <code>create_customer()</code></b> —— 這是刻意的。走函式才有地方擋「手機格式」「重複」「姓名少於兩個字」這三件事。姓名至少兩個字是硬性的：登記只有一個字（例如「王」）的話，<b>任何含「王」的名字都會通過綁定</b>。<br><br>☢️ 兩張新檢視表（<code>staff_signups</code>／<code>staff_customers</code>）都<b>不能</b>加 <code>security_invoker</code> —— 牆是 <code>is_staff()</code>，不是底層權限（第 47 步剛付過這個學費）。',
