  ...
  acts: [
    ...
    /* ══════════ 第五幕 ══════════ */
    {
      ...
      steps: [
        ...
        { n:63, t:'官網那套「預約」是假的 —— 拆掉，改成導到 LINE', where:'前端', done:false,
        ...
        { n:67, t:'職員專屬圖文選單 —— 同一格，客人傳訊息、教練進後台', where:'LINE 設定 ＋ 前端', done:false,
          ...
          warn:'☢️ <b>建立選單時 <code>selected</code> 必須是 <code>false</code>。</b>設成 true 會把它變成<b>全部使用者的預設選單</b> —— 203 位客人當場看到「STAFF 入口」，而且第六格會把他們帶到教練頁面（進得去看不到東西，但那是驚嚇）。<br><br>☢️ <b>上傳底圖的網域是 <code>api-data.line.me</code>，不是 <code>api.line.me</code>。</b>用錯會回 404，而 404 看起來像「選單不存在」—— 其實選單好好的，只是傳錯地方。<br><br>☢️ <b>PowerShell 5.1 兩個坑</b>：① 預設可能用 TLS 1.0，LINE 只收 1.2 以上，不設會是「無法建立 SSL/TLS 安全通道」，那個訊息完全看不出跟 TLS 有關；② <code>Invoke-RestMethod</code> 送字串時預設不是 UTF-8，<b>中文的選單名稱會變亂碼</b> —— 要自己轉成 UTF-8 位元組再送。<br><br>☢️ <b>2500 ÷ 3 除不盡</b>（833.33）。中間那格給 834，總和才剛好 2500。<b>差一個像素 LINE 會拒絕整張選單。</b><br><br>☢️☢️ <b>userId 可能對不上。</b>我們手上的七個 userId 來自 <b>LINE Login channel（2011063116）</b>，而圖文選單屬於 <b>Messaging API channel（2009245280）</b>。LINE 的 userId 是<b>以 provider 為單位</b>的 —— 兩個 channel 在同一個 provider 底下才會是同一組 ID。<b>如果全部綁定都回 404，就是這個原因</b>，那時要改從 Messaging API 的 webhook 收 userId。腳本會把這件事直接印出來。<br><br>☢️ <b>圖上那行「傳訊息給櫃檯」對職員會變成謊話</b> —— 他按下去是進教練頁面，不是傳訊息。所以 <code>staff.html</code> 加了一顆<b>「回聊天室傳訊息給櫃檯」</b>（只在 LINE App 裡出現，按下去 <code>liff.closeWindow()</code>）。<b>網頁沒有權限代替使用者發訊息</b> —— 我們刻意沒要 <code>chat_message.write</code>，那個權限會讓 LIFF 視窗縮不到底下。能做的是把視窗關掉，人就回到聊天室。<br><br>☢️ <b>兩張圖要一起改。</b>只改一張的話，教練跟客人講的畫面會對不上。',
          note:'<b>2026-08-19 準備完成，等 Jerec 執行。</b>檔案：<code>assets/brand/richmenu-staff-source.html</code>（新）、<code>assets/brand/shot-staff.py</code>（新）、<code>assets/brand/fff-richmenu-staff.png</code>（新，448 KB）、<code>tools/setup-staff-richmenu.ps1</code>（新）、<code>line-prototype/staff.html</code>。<br><br><b>底圖驗證</b>：2500×1686 ✓　448 KB（上限 1024）✓　「★ STAFF 入口」在手機上換算 <b>9.7pt</b>（副標是 11pt，比它小一級但還讀得到）✓　格線與客人版完全一致 ✓<br><br><b><code>staff.html</code> 驗證</b>：<ul><li>在 LINE App 裡 → 出現「回聊天室傳訊息給櫃檯」，按下去確實呼叫 <code>liff.closeWindow()</code> ✓</li><li>在瀏覽器（桌機）→ <b>不出現</b>（那裡沒有聊天室可以回）✓</li><li>零 JS 錯誤 ✓</li></ul><b>名單</b>：七位職員的 LINE userId 已經寫進腳本（櫃檯平板還沒開通，不在名單上）。',
