...
> ☢️ **2026-08-22：這張表已經被 v3 取代。** Jerec 指定改順序，底圖重畫成
> 淺色襯線的「修復風」。**新的對應在下面附錄六之十四**，而且
> **可點區域是綁在「位置」上、不是綁在圖案上** —— 換圖沒改動作的話，
> 客人按「私人課」會進團體課訂課頁，而且不會有任何錯誤訊息。
...
## 附錄六之十四：2026-08-22（圖文選單 v3 · 換順序＋改風格）

Jerec 2026-08-22：
① 六格順序改成「私人課／團體課／私人團體／我的預約／價目表／聯絡我們」
② 拿掉左上角那條裝飾用白邊
③ 以品牌 LOGO 發想，走**輕盈質感／修復風**，字體改**襯線體**

### 一　☢️ 新的六格對應（換圖之後一定要重設動作）

| 格 | 位置 | 標題 | 動作 |
|---|---|---|---|
| A | 左上 | 私人課 | `liff.line.me/2011063116-QOxXN30h/PT-booking.html` |
| B | 中上 | 團體課 | `…/GT-booking.html` |
| C | 右上 | 私人團體 | `…/PGT-booking.html` |
| D | 左下 | 我的預約 | `…/GT-booking.html?tab=m` |
| E | 中下 | 價目表 | `…/pricing.html` |
| F | 右下 | 聯絡我們 | **文字**「我想詢問」 |

☢️ **前四格全部換了動作。** 只換底圖不改動作 = 每一個入口都指到錯的地方，
而且**不會報錯**。順序永遠是：先在 LINE 後台把六個動作改好、確認、才儲存。

☢️ 換選單之前一樣要先把**舊選單的使用期間結束日改成當天**，
否則 LINE 會擋：「此使用期間已設有其他圖文選單」（附錄六之九踩過）。

### 二　設計上改了什麼，以及為什麼

* **深底白字 → 淺底深字。** 修復風的核心是留白與低彩度，深藍滿版做不出來。
  ☢️ 但對比不能因此變差 —— 現在由 `shot.py` **實際驗算**，不到 4.5:1 直接中斷。
* **襯線體**（Noto Serif CJK TC）。☢️ 襯線的橫畫細，縮到手機上（15.6%）容易糊，
  所以副標 72px → 84px（手機 13pt）、字重用 Medium。**字級只能加不能減。**
* **拿掉 `.lead::before`**（那條裝飾白邊）。它想說「這格是主角」，
  但主角已經由順序表達了 —— 第一格就是主角。
* **LOGO 的緞帶變成底紋。**
  ☢️ 第一版畫成**兩道一模一樣、並排的 S**，結果整張圖看起來是**兩個問號**。
  改成**兩端都出框的長弧線**：看得到的只有中段，沒有起點也沒有終點，
  就不會被讀成任何字。

### 三　☢️ `shot.py` 多了一道保險絲，而且它才是重點

v2 的對比只寫在註解裡「都驗過」—— **那等於沒驗**：
改了 CSS 的顏色，註解不會跟著變，而下一個人會相信它。

v3 改成：**從 `richmenu-source.html` 裡找那些色碼**，找不到就中斷。
換了顏色卻忘了更新檢查，會當場被擋下來，不會默默通過。
