...
> 📌 **完整的 41 步清單在 `plan/` 資料夾**，用瀏覽器打開 `plan/fff-roadmap-steps.html`。
...
- 新增了 **第 36 步「切換日」**：停業一天，正式搬家（週三只有一堂課，是天然的分水嶺）
- 步驟總數 **36 → 39**
...
> 這一節記的是**只存在於老闆腦子裡的規則**。
> 它不是技術文件 —— 但少了它，第 38 步的點名核銷和之後的 ERP 薪資模組都寫不出來。
...
> ⚠️ **`n` 是「實到人數」，不是「報名人數」。**
> 所以第 38 步的點名核銷不只是扣客人的課 —— **它同時是教練薪資的原始憑證**。
> 點名漏掉一個人，教練就少領 100 元。
...
| 差異 | 處理方式 |
|---|---|
| 舊系統有容量 12 的課，我們 14 堂全是 10 | 第 25 步人工核對 |
| 程度舊系統寫全字、我們用縮寫 | 建一組對照表 |
| 舊 `Classes` 只有 5 位教練，沒有簡基城 | 第 25 步確認他帶不帶團體課 |
| 81 人中 13 人完全沒有帳本紀錄 | 確認是「0 堂」還是「沒建帳」 |
| `assignedCoach`、`email` 幾乎全空 | 不搬 |
